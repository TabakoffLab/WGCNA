...
gtfFile = read.table(file="/Volumes/saba/BNLx.SHR/RNA-Seq.Brain.total/reconstruction/reconstruct.total.brain.FINAL.26Aug14.gtf",sep="\t", header=FALSE)
...
#139,210 unique probesets
length(unique(exonInfo[,"gene_id"]))
#13,608 unique transcript clusters
...
pdf(file = "/Volumes/saba/wgcna.PhenoGen/images/sampleClustering.brain.03Feb15.pdf", width = 12, height = 9);
par(cex = 0.6);
par(mar = c(0,4,2,0))
...
write.table(herits, file=paste(data.path,"herits.clusters.HXB.brain.03Feb15.txt",sep=""), sep="\t",col.names=TRUE,row.names=FALSE,quote=FALSE)
